 ...
 * 简化版唇形同步参数
 */
export interface SimpleLipSyncParams {
 ...
 * 表情类型
 */
export type MoodType = 'neutral' | 'happy' | 'thinking' | 'serious' | 'surprised'
 ...
 * @returns 动画帧数组
 */
export function generateLipSyncFrames(text: string, duration: number): LipSyncFrame[] {
  if (!text || !duration) {
    return []
  }
 ...
 * @returns 嘴部开合度 (0-1)
 */
export function getMouthOpenness(frames: LipSyncFrame[], currentTime: number): number {
 ...
 * @returns 动画参数
 */
export function generateSimpleLipSync(text: string): SimpleLipSyncParams {
  if (!text) {
    return {
      totalDuration: 0,
      speaking: false,
 ...
 * @returns 嘴部开合度 (0-1)
 */
export function updateMouthAnimation(text: string, progress: number): number {
 ...
 * @returns 嘴部基础状态
 */
export function getMoodMouthState(mood: MoodType = 'neutral'): MouthState {
  const moodStates: Record<MoodType, MouthState> = {
    neutral: {
      baseOpenness: 0.2,
      cornerUp: 0,
      width: 1
